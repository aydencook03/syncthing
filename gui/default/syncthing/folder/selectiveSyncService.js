...
angular.module('syncthing.core')
    .factory('selectiveSyncService', ['$http', '$q', function ($http, $q) {
        ...
        // _state[folderId] = { enabled: bool, paths: Set, touched: bool }
        var _state = {};
        ...
        // Load current state from the backend (.stselective + .stignore include check).
        function load(folderId) {
            return $http.get(urlbase + '/db/selective?folder=' + encodeURIComponent(folderId))
                .then(function (r) {
                    var s = st(folderId);
                    // Only import from disk if the user hasn't made changes yet.
                    if (!s.touched) {
        ...
        // Save: write .stselective and manage the #include line in .stignore.
        function save(folderId) {
            var s = st(folderId);
            var paths = s.enabled ? Array.from(s.paths).sort() : [];

            // 1. Write (or clear) .stselective
            return $http.post(
                urlbase + '/db/selective?folder=' + encodeURIComponent(folderId),
                { paths: paths }
            ).then(function () {
                // 2. Read current .stignore
                return $http.get(urlbase + '/db/ignores?folder=' + encodeURIComponent(folderId));
            }).then(function (r) {
                var lines = (r.data && r.data.ignore) || [];
                var hasInclude = lines.indexOf(INCLUDE_LINE) !== -1;

                if (s.enabled && !hasInclude) {
                    lines = [INCLUDE_LINE].concat(lines);
                } else if (!s.enabled && hasInclude) {
