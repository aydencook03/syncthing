    ...
    .directive('selectiveSyncBrowser', ['selectiveSyncService', '$http', '$timeout', function (selectiveSyncService, $http, $timeout) {
        ...
        return {
            ...
            link: function (scope, element) {
                ...
                function browseUrl(prefix) {
                    var url = urlbase + '/db/browse?folder=' + encodeURIComponent(scope.folderId) + '&levels=1';
                    if (prefix) {
                        var p = prefix.replace(/^\/+/, '');
                        if (p) url += '&prefix=' + encodeURIComponent(p);
                    }
                    return url;
                }
                ...
                function toNodes(items, parentPath) {
                    return (items || []).map(function (item) {
                        ...
                        var node = { title: item.name, key: path, folder: isDir, lazy: isDir,
                                     selected: sel, data: { path: path } };
                ...
                function applySelection(node) {
                    if (!node.data || !node.data.path) return;
                ...
                function loadRoot() {
                    ...
                    selectiveSyncService.load(scope.folderId).then(function () {
                        ...
                        if (tree) {
                            tree.reload(nodes).done(function () { tree.visit(applySelection); });
                        } else {
                            $timeout(function () { mountTree(nodes); });
                        }
                    }, function (err) {
                        scope.loading = false;
                        scope.error = (err && (err.status === 500 || err.status === 503))
                            ? 'File tree not available yet.'
                            : 'Failed to load file tree (error ' + (err && err.status) + ').';
                    });
                }
                ...
                function mountTree(nodes) {
                    ...
                    $el.fancytree({
                        ...
                        lazyLoad: function (event, data) {
                            ...
                            var def = $.Deferred();
                            $http.get(browseUrl(path)).then(function (r) {
                                cache[path] = r.data || [];
                                def.resolve(toNodes(cache[path], path));
                            }, function () { def.reject(); });
                            data.result = def.promise();
                        },
                        init: function (event, data) { data.tree.visit(applySelection); },
                        loadChildren: function (event, data) { data.node.visit(applySelection); },
                        select: function (event, data) { onSelect(data.node); },
                ...
                function explodeAncestor(node) {
                    ...
                    var chain = ancestor;
                    path.slice(ancestor.length).split('/').filter(Boolean).forEach(function (seg) {
                        var next = chain + '/' + seg;
                ...
                scope.$on('selectiveSyncTabVisible', function (evt, data) {
                    if (data.folderId === scope.folderId && !tree) loadRoot();
                });
